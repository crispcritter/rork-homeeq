...
export const [HomeProvider, useHome] = createContextHook(() => {
  ...
  const addBudgetItem = useCallback((item: BudgetItem) => {
    void listMutate<BudgetItem>(STORAGE_KEYS.budgetItems, ['budgetItems'], (items) => {
  ...
  const addTrustedPro = useCallback((pro: TrustedPro) => {
    void listMutate<TrustedPro>(STORAGE_KEYS.trustedPros, ['trustedPros'], (items) => [...items, pro]);
  }, [listMutate]);
  ...
  return useMemo(() => ({
    ...
    updateBudgetItem, deleteBudgetItem, addTrustedPro, updateTrustedPro, deleteTrustedPro,
    addProPrivateNote, removeProPrivateNote, updateProPrivateNote, linkApplianceToPro,
